...
private struct RightCluster: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            // native drag-reorder. The hit zone is the full 38×38 frame
            ...
            Button {
                ...
                Image(systemName: "text.alignleft")
                    .font(.system(size: 14))
                    .foregroundColor(showTranscript ? accent : Ink.primary)
                    .frame(width: 38, height: 38)
                    .background(
                        Circle().fill(showTranscript ? accent.opacity(0.1) : .clear)
                    )
                    .contentShape(Circle())
            }
            .buttonStyle(.plain)
