...
enum ShareExport {
    ...
    /// receivers can click through without further context.
    ...
    ///
    ///     [Episode Title](https://…) — Show · 2026-05-13
    static func shareMarkdown(for ep: Episode) -> String {
        let dateStr = Fmt.date(ep.pubDate)
        let show = ep.show?.title ?? ""
        if let url = shareURL(for: ep) {
            return "[\(ep.title)](\(url.absoluteString))\(show.isEmpty ? "" : " — \(show)") · \(dateStr)"
        }
        // No URL — still useful as a citation.
        return "**\(ep.title)**\(show.isEmpty ? "" : " — \(show)") · \(dateStr)"
    }
